// js/main.js (hotfix: non-blocking sprite load + self-healing DOM)
...
async function ensureDom() {
  ...
  if (!canvas) {
    ...
    canvas.id = "g"; canvas.width = W; canvas.height = H;
  ...
  if (!elLeaderboard) {
    ...
    elLeaderboard.id = "ui-leaderboard"; elLeaderboard.textContent = "未設定";
...
window.addEventListener("keydown", (e)=>{ if(!e.repeat){ keys.add(e.key); } if(["ArrowUp","ArrowDown","ArrowLeft","ArrowRight"," "].includes(e.key)) e.preventDefault(); });
...
function attachPointerHandlers(){
  canvas.addEventListener("pointerdown",(e)=>{ pointerDown=true; const p=canvasPos(e); pointerX=p.x; pointerY=p.y; unlockAudio(); });
  window.addEventListener("pointerup",()=> pointerDown=false);
  canvas.addEventListener("pointermove",(e)=>{ if(!pointerDown) return; const p=canvasPos(e); pointerX=p.x; pointerY=p.y; });
}
...
async function prepareBgmFromConfig(cfg){
  ...
  for (const [key,spec] of Object.entries(cfg.audio.bgm)){
    const src=spec.src||spec.url||spec.file; if(!src) continue;
    bgm.tracks.set(key,{src,loopStart:Number(spec.loopStart??0),loopEnd:Number(spec.loopEnd??0),buf:null});
  }
}
...
async function crossfadeTo(key){
  ...
  const {loopStart=0,loopEnd=0}=bgm.tracks.get(key); const nxt=startLoopingSource(buf,loopStart,loopEnd); bgm.next={key,...nxt};
...
function createSpawnerAdapter(cfg){
  ...
  return {
    update(dt,now){
      const s=cfg.spawns||{}, bscale=cfg?.tuning?.bulletSpeedScale ?? 1.0;
      function push(b){ if(b) bullets.push(b); }
      last.rain+=dt; if(last.rain>=(s.rainEvery??3.8)){ last.rain=0; const n=Math.max(8,Math.floor(14*difficultyMul(now))); for(let i=0;i<n;i++){ const x=(W/n)*(i+0.5), y=-10, v=90*(0.9+Math.random()*0.3)*bscale; push(new NormalBullet(x,y,0,v)); } }
      last.side+=dt; if(last.side>=(s.sideEvery??1.8)){ last.side=0; const left=Math.random()<0.5, y0=60+Math.random()*(H-120), c=Math.max(4,Math.floor(8*difficultyMul(now))); for(let i=0;i<c;i++){ const x=left?-10:W+10, vx=(left?140:-140)*(0.9+Math.random()*0.3)*bscale, vy=(Math.random()*2-1)*20; push(new FastBullet(x,y0+i*8,vx,vy)); } }
      last.ring+=dt; if(last.ring>=(s.ringEvery??9.0)){ last.ring=0; const cx=80+Math.random()*(W-160), cy=80+Math.random()*(H-160), n=24, spd=130*bscale; for(let i=0;i<n;i++){ const a=(i/n)*Math.PI*2; push(new NormalBullet(cx,cy,Math.cos(a)*spd,Math.sin(a)*spd)); } }
...
// ----------------------------------------
// Zones
function valAsPx(v,total){ if(v==null) return 0; if(v>0 && v<=1) return v*total; return v; }
...
function drawZones(t){ const z=getZonesNow(t); ctx.fillStyle="rgba(20,200,120,0.10)"; ctx.fillRect(0,z.safeTop,W,z.safeH);
  ...
  const bm=config?.bonusMove||{}, cx=bm.cx??(W/2), cy=bm.cy??(H*0.45), R=bm.r??70;
  ctx.strokeStyle="rgba(250,210,60,0.25)"; ctx.beginPath(); ctx.arc(cx,cy,R,0,Math.PI*2); ctx.stroke(); }
...
function loop(ts){
  ...
  if(state===TITLE){
    if(keys.has("Enter")||pointerDown){ startGame(); pointerDown=false; }
  } else if(state===PLAY){
    ...
    if((performance.now()>=allowReturnAt) && (keys.has("Enter")||pointerDown)){ pointerDown=false; enterTitle(); }
  }
...
(async function boot(){
  try{
    ...
    // ★スプライトは「待たずに」読み込み（失敗してもフォールバック描画に任せる）
    loadBulletSprites().catch(err=>console.warn("sprite load failed (continue with fallback):", err));

    // Spawner
    spawner = createSpawnerAdapter(config);

    // デバッグトグル
    window.addEventListener("keydown",(e)=>{ if(e.key==="F2") showDebug=!showDebug; });

  }catch(err){
    console.error(err);
    if(ctx){ ctx.fillStyle="#fff"; ctx.font="16px monospace"; ctx.fillText("BOOT ERROR: "+err.message, 12, 20); }
    else { alert("BOOT ERROR: "+err.message); }
  }
})();
