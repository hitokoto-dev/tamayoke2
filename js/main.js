// js/main.js (rank endpoint override 対応版)
...
async function domReady(){ if(document.readyState==="loading") await new Promise(r=>document.addEventListener("DOMContentLoaded",r,{once:true})); }
...
await domReady();
canvas=document.getElementById("g")||(()=>{const c=document.createElement("canvas");c.id="g";c.width=W;c.height=H;(document.getElementById("stage")||document.body).appendChild(c);return c;})();
...
async function prepareBgmFromConfig(cfg){ if(!cfg?.audio?.bgm) return; bgm.crossfadeMs=Math.max(0,cfg.audio.crossfadeMs??400);
  for(const [k,s] of Object.entries(cfg.audio.bgm)){ const src=s.src||s.url||s.file; if(!src) continue; bgm.tracks.set(k,{src,loopStart:+(s.loopStart??0),loopEnd:+(s.loopEnd??0),buf:null}); } }
...
async function crossfadeTo(key){ if(!AC||AC.state!=="running"||!bgm.tracks.has(key)||bgm.current?.key===key) return;
  const buf=await ensureTrackBuffer(key); if(!buf) return; const {loopStart=0,loopEnd=0}=bgm.tracks.get(key); const nxt=startLoopingSource(buf,loopStart,loopEnd); bgm.next={key,...nxt};
...
function createSpawnerAdapter(cfg){
  ...
  return { update(dt,now){
    const s=cfg.spawns||{}, b=cfg?.tuning?.bulletSpeedScale ?? 1.0; function push(x){ if(x) bullets.push(x); }
    ...
    last.side+=dt; if(last.side>=(s.sideEvery??1.8)){ last.side=0; const L=Math.random()<0.5, y0=60+Math.random()*(H-120), c=Math.max(4,Math.floor(8*difficultyMul(now)));
      for(let i=0;i<c;i++){ const x=L?-10:W+10, vx=(L?140:-140)*(0.9+Math.random()*0.3)*b, vy=(Math.random()*2-1)*20; push(new FastBullet(x,y0+i*8,vx,vy)); } }
    last.ring+=dt; if(last.ring>=(s.ringEvery??9.0)){ last.ring=0; const cx=80+Math.random()*(W-160), cy=80+Math.random()*(H-160), n=24, spd=130*b; for(let i=0;i<n;i++){ const a=(i/n)*Math.PI*2; push(new NormalBullet(cx,cy,Math.cos(a)*spd,Math.sin(a)*spd)); } }
...
function drawZones(t){ const z=getZonesNow(t); ctx.fillStyle="rgba(20,200,120,0.10)"; ctx.fillRect(0,z.safeTop,W,z.safeH);
  const cols=config?.zones?.safeCols??0; if(cols>1){ ctx.strokeStyle="rgba(60,220,160,0.10)"; for(let i=1;i<cols;i++){ const x=(W/cols)*i; ctx.beginPath(); ctx.moveTo(x,z.safeTop); ctx.lineTo(x,H); ctx.stroke(); } }
...
function drawPlayer(){ ctx.save(); ctx.translate(player.x,player.y); ctx.fillStyle="#4cf"; const s=player.size;
  ctx.beginPath(); ctx.moveTo(0,-s); ctx.lineTo(s*0.7,0); ctx.lineTo(0,s); ctx.lineTo(-s*0.7,0); ctx.closePath(); ctx.fill();
  ctx.strokeStyle="rgba(255,255,255,0.35)"; ctx.beginPath(); ctx.arc(0,0,player.hitR,0,Math.PI*2); ctx.stroke(); ctx.restore(); }
...
function updatePlayer(dt){ let dx=0,dy=0;
...
let prevTs=performance.now(), fps=60, state=TITLE, allowReturnAt=0, timePlay=0, score=0, showDebug=DEBUG;
function loop(ts){
  const dt=Math.min(1/20, Math.max(0,(ts-prevTs)/1000)); prevTs=ts; fps=0.9*fps+0.1*(1/dt);

  if(state===TITLE){ if(keys.has("Enter")||pointerDown){ pointerDown=false; state=PLAY; timePlay=0; score=0; bullets.length=0; crossfadeTo("play").catch(()=>{}); } }
  else if(state===PLAY){
    ...
    if(checkCollision()){ const finalScore=score|0; submitIfBest(finalScore).catch(()=>{}); state=OVER; allowReturnAt=performance.now()+700; bullets.length=0; crossfadeTo("title").catch(()=>{}); }
  } else if(state===OVER){ if((performance.now()>=allowReturnAt) && (keys.has("Enter")||pointerDown)){ pointerDown=false; state=TITLE; timePlay=0; bullets.length=0; } }
  ...
  if(showDebug){ ctx.font="12px monospace"; ctx.textAlign="left"; ctx.textBaseline="top"; ctx.fillStyle="rgba(255,255,255,0.85)"; ctx.fillText(`V=${BUILD_V} state=${state} bullets=${bullets.length}`, 8, H-28); }
...
(async function boot(){
  try{
    ...
    // rank endpoint: config → 上書き（window.RANK_ENDPOINT があれば優先）
    const epFromConfig = (config?.rankApi?.endpoint || "").trim();
    const endpoint = (globalThis.RANK_ENDPOINT && String(globalThis.RANK_ENDPOINT).trim()) || epFromConfig;
    rank = new Rank(endpoint);
    // 最初に描画
    renderLeaderboard(rank, elLeaderboard).catch(()=>{});

    // audio / sprites / spawner
    makeAC(); await prepareBgmFromConfig(config);
    loadBulletSprites().catch(()=>{});
