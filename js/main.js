...
let bullets=[], state=TITLE, allowReturnAt=0, time=0, score=0;

// かんたん弾
const timers={ rain:0, side:0, ring:0 };
function spawn(dt){
  timers.rain+=dt; timers.side+=dt; timers.ring+=dt;
  if(timers.rain>=0.16){ timers.rain=0; const x=20+Math.random()*(W-40); bullets.push({x,y:-10,vx:0,vy:140+Math.random()*50,r:6,alive:true}); }
  if(timers.side>=1.2){ timers.side=0; const left=Math.random()<0.5, y=60+Math.random()*(H-220); for(let i=0;i<6;i++){ bullets.push({x:left?-10:W+10,y:y+i*10,vx:left?160:-160,vy:(Math.random()*2-1)*20,r:6,alive:true}); } }
  if(timers.ring>=2.4){ timers.ring=0; const cx=80+Math.random()*(W-160), cy=80+Math.random()*(H-220), n=18, spd=120; for(let i=0;i<n;i++){ const a=i/n*Math.PI*2; bullets.push({x:cx,y:cy,vx:Math.cos(a)*spd,vy:Math.sin(a)*spd,r:6,alive:true}); } }
}
function update(dt){
  let dx=0,dy=0;
  ...
  for(const b of bullets){ const rr=(b.r+player.hitR), dx=b.x-player.x, dy=b.y-player.y; if(dx*dx+dy*dy<=rr*rr) return true; }
  return false;
}

// ---------- 描画 ----------
function drawBG(){ const g=ctx.createLinearGradient(0,0,0,H); g.addColorStop(0,"#0b1020"); g.addColorStop(1,"#101a35"); ctx.fillStyle=g; ctx.fillRect(0,0,W,H); ctx.fillStyle="rgba(20,200,120,0.10)"; ctx.fillRect(0,H-90,W,90); }
...
function drawDebug(){ if(!DEBUG) return; ctx.font="12px monospace"; ctx.textAlign="left"; ctx.textBaseline="top"; ctx.fillStyle="rgba(255,255,255,0.85)"; ctx.fillText(`V=${BUILD_V} state=${state} bullets=${bullets.length}`, 8, H-20); }
...
async function submitScore(finalScore){
  ...
  const best = loadBest();
  const mustSubmit = !alreadySubmitted;                 // ★初回は必ず送信
  const isBest = (finalScore|0) > best;

  if(!mustSubmit && !isBest) return;                    // 2回目以降はベストのみ送信
  if(isBest) saveBest(finalScore|0);                    // ベストを更新

  let name = loadPlayerName();
  if(!name){ name=(prompt("名前（16文字まで）を入力してください","YOU")||"YOU").trim().slice(0,16); savePlayerName(name); }

  const prevText = elLeaderboard.textContent;
  elLeaderboard.textContent = "SUBMITTING...";

  const res = await rank.submit(name, finalScore|0);
  if(res.status==="ok"){
    localStorage.setItem(SUBMITTED_KEY,"1");
    await renderLeaderboard(rank, elLeaderboard);
  }else{
    console.error(res.error||"submit failed");
    elLeaderboard.textContent = prevText || "通信エラー";
  }
}
...
function loop(ts){
  ...
  if(state===TITLE){ if(keys.has("Enter")||pointerDown){ pointerDown=false; state=PLAY; time=0; score=0; bullets.length=0; } }
  else if(state===PLAY){
    time+=dt; score += 10*dt;
    const hit=update(dt);
